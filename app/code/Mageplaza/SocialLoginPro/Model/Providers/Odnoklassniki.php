...
class Odnoklassniki extends \Hybrid_Provider_Model_OAuth2
{
    ...
    function getUserProfile()
    {
        ...
        // Signature
        $sig = md5('application_key=' . $this->config['public_key'] . 'fields=' . $fields . 'method=users.getCurrentUser' . md5($this->api->access_token . $this->api->client_secret));
        // Signed request
        $response = $this->api->api('?application_key=' . $this->config['public_key'] . '&fields=' . $fields . '&method=users.getCurrentUser&sig=' . $sig);
