...
namespace Devbatch\LayeredNavigation\Model\Layer\Filter;
...
class Attribute extends AbstractFilter
{
    ...
    public function apply(\Magento\Framework\App\RequestInterface $request)
    {
        ...
        if (count($attributeValue) > 0) {
            ...
            for ($i=0; $i < count($attributeValue); $i++) { 
                $productCollection->addAttributeToFilter($attribute->getAttributeCode(), ['finset' => $attributeValue[$i]]);
            }
    ...
    protected function _getItemsData()
    {
        ...
        foreach ($options as $option) {
            ...
            $check2 = 0;
                foreach ($testCollection as $key) {
                    $arr = explode(',', $key[$attribute->getAttributeCode()]) ;
