...
class InstallSchema implements InstallSchemaInterface
{
    ...
    public function install(SchemaSetupInterface $setup, ModuleContextInterface $context)
    {
		 ...
		 */
		$setup->getConnection()->dropTable($setup->getTable('magebest_brand_group'));
		$setup->getConnection()->dropTable($setup->getTable('magebest_brand'));
		$setup->getConnection()->dropTable($setup->getTable('magebest_brand_store'));

 		/**
 		 * Create table 'magebest_brand_group'
 		 */
 		$table = $setup->getConnection()
 		->newTable($setup->getTable('magebest_brand_group'))
 		->addColumn(
 			'group_id',
 			Table::TYPE_INTEGER,
 			11,
        ...
        $setup->getConnection()->createTable($table);
 		...
 		/**
 		 * Create table 'magebest_brand'
 		 */
 		$table = $setup->getConnection()
 		->newTable($setup->getTable('magebest_brand'))
 		->addColumn(
 			'brand_id',
 			Table::TYPE_INTEGER,
 			null,
 		...
 		->addColumn(
            ...
            ->addIndex(
                $setup->getIdxName('magebest_brand', ['group_id']),
                ['group_id']
                )
            ->addForeignKey(
                $setup->getFkName('magebest_brand', 'group_id', 'magebest_brand', 'group_id'),
                'group_id',
                $setup->getTable('magebest_brand_group'),
                'group_id',
 		...
 		/**
         * Create table 'magebest_brand_store'
         */
 		$table = $setup->getConnection()
 		->newTable($setup->getTable('magebest_brand_store'))
 		->addColumn(
 			'brand_id',
 			Table::TYPE_INTEGER,
 			null,
 		...
 		->addIndex(
 			$setup->getIdxName('magebest_brand_store', ['store_id']),
 			['store_id']
 			)
 		->addForeignKey(
 			$setup->getFkName('magebest_brand_store', 'brand_id', 'magebest_brand', 'brand_id'),
 			'brand_id',
 			$setup->getTable('magebest_brand'),
 			'brand_id',
 			Table::ACTION_CASCADE
 			)
 		->addForeignKey(
 			$setup->getFkName('magebest_brand_store', 'store_id', 'store', 'store_id'),
 			'store_id',
