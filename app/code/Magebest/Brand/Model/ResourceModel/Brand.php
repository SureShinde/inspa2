...
class Brand extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    protected function _construct(){
    	$this->_init('magebest_brand','brand_id');
    }
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        if ($object->getStoreId()) {
            ...
            $select->join(
                ['magebest_brand_store' => $this->getTable('magebest_brand_store')],
                $this->getMainTable() . '.brand_id = magebest_brand_store.brand_id',
                []
                )->where(
                'status = ?',
                1
                )->where(
                'magebest_brand_store.store_id IN (?)',
                $storeIds
                )->order(
                'magebest_brand_store.store_id DESC'
                )->limit(
                1
                );
            }
    ...
    protected function _getLoadByIdentifierSelect($identifier, $store, $isActive = null)
    {
        $select = $this->getConnection()->select()->from(
            ...
            ['cps' => $this->getTable('magebest_brand_store')],
    ...
    protected function _beforeDelete(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $this->getConnection()->delete($this->getTable('magebest_brand_store'), $condition);

        $condition = ['brand_id = ?' => (int)$object->getId()];
        $this->getConnection()->delete($this->getTable('magebest_brand_product'), $condition);
    ...
    protected function _afterSave(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $table = $this->getTable('magebest_brand_store');
        ...
        if(null !== ($object->getData('products'))){
            $table = $this->getTable('magebest_brand_product');
    ...
    public function getBrandIdByName($brand_name = '') {
        if($brand_name) {
            $brand_id = null;
            $table = $this->getTable('magebest_brand');
    ...
    protected function _afterLoad(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        if ($id = $object->getId()) {
                ...
                ->from($this->getTable('magebest_brand_product'))
                ->where(
                    'brand_id = '.(int)$id
                    );
    ...
    public function lookupStoreIds($brandId)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable('magebest_brand_store'),
            'store_id'
            )
        ->where(
            'brand_id = ?',
